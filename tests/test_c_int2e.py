...
class Int2e_cTestCase(unittest.TestCase):
    ...
    def test_get_v2e3(self):
        X = 0.52918  # 単位変換: angstrom -> a0
        mol = pyscf.gto.Mole()
        mol.build(atom='H 0 0 %f; I 0 0 %f' % (-0.7 * X, 0.7 * X),
                  basis="sto3g")
        V1 = mol.intor('int2e')

        M = mscf.mole.mole.Mole([['H', 0, 0, -0.7], ['I', 0, 0, 0.7]], 'sto3g')
        V = int2e_c.c_get_v2e(M)
        self.assertEqual(V1.shape, V.shape)
        rel_tol = 2e-4
        abs_tol = 1e-12
